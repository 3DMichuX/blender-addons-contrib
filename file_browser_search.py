...
class SearchResultsItem(bpy.types.PropertyGroup):
    name = bpy.props.StringProperty(name="File name",
																default="")
    displayName = bpy.props.StringProperty(name="Display name",
                                                                    default="")

def file_browser_search(self, context):
#  maximum number of files to search for
    MAX_FILES = 100
#  maximum number of directories to check (to avoid to much nesting)
    MAX_DIRS = 100
    
#	get properties from file browser
#  filter
	filter = context.window_manager.file_browser_search_filter
#	last directory
    last_directory = context.window_manager.file_browser_search_last_dir
#	search results
    search_results = context.window_manager.file_browser_search_search_results    
    
#  clear list
    del search_results[:]
#  if empty filter finish    
    if (filter == ""):
        return None
...
#  setup regular expression pattern
    pattern = ""    
    special = ('\\', '.', '^', '$', '*', '+', '?', '{', '}', '[', ']', '|', '(', ')')
#	replace all special characters in name filter
    for c in special:
        filter = filter.replace(c, '\\'+c)
#	create regular expression based on name filter
    if ('*' in filter):
...
def file_search(self, context):
#	get properties from file browser
    search_tree = context.Scene.
...
def blend_data_from_file(file, part):
...
def blend_data_search(self, context):
...
def file_browser_search(self, context):
    if(context.active_operator.bl_idname=="WM_OT_link_append"):
        return file_browser_search(self, context)
    else:
        return file_browser_search(self, context)
...
#   global properties set in Scene type
    bpy.types.Scene.file_browser_search_autoexecute = bpy.props.BoolProperty(name="Open Automatically", default=True)
    bpy.types.Scene.file_browser_search_hide_extensions = bpy.props.BoolProperty(name="Hide Extensions", update=filtered_search)
    bpy.types.Scene.file_columnsnumber = bpy.props.IntProperty(name="Number of Columns", default=2, min=1, max=15, update=filtered_search)
    
#  temporary properties    
    bpy.types.WindowManager.file_browser_search_search = bpy.props.StringProperty(update=filtered_search)
    bpy.types.WindowManager.file_browser_search_last_dir = bpy.props.StringProperty()
    bpy.types.WindowManager.file_browser_search_search_subdirectories = bpy.props.BoolProperty(name="Search Subdirectories", update=filtered_search)
